...
async function main() {
  ...
  try {
    ...
    // Step 8: Open Ações dropdown and click PDF
    console.log('Step 8: Opening Ações dropdown...');

    // Click the Ações button to open the dropdown
    ...
    await page.screenshot({ path: 'step8-acoes-open.png', fullPage: true });

    // Step 9: Click PDF using JavaScript (avoids visibility issues)
    console.log('Step 9: Clicking PDF download...');

    const pdfClicked = await page.evaluate(() => {
      // Try clicking by ID first
      const pdfLink = document.getElementById('download_txt2');
      if (pdfLink) {
        pdfLink.click();
    ...
    console.log('PDF click result:', JSON.stringify(pdfClicked));

    await page.screenshot({ path: 'step9-pdf-clicked.png', fullPage: true });
